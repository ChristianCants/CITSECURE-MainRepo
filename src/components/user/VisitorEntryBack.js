...
import { useNavigate } from 'react-router-dom'; // Use navigate instead of withRouter
...
class VisitorEntryBackGate extends Component {

    constructor(props) {
        super(props);
        this.state = {
          selectedGate: 'Back Gate', 
          firstName: '',
          lastName: '',
          purpose: '',
          cardNo: '',
  ...
  render() {
    ...
    const isFormFilled =firstName && lastName && purpose && cardNo && buildingToVisit && selectedOffice && assignedPerson  && visitorimage2;
...
const VisitorEntryBack = () => {
  const navigate = useNavigate(); // Use the hook to pass navigation as a prop
  return <VisitorEntryBackGate navigate={navigate} />;
};

export default VisitorEntryBack;