...
const RTLBuilding = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('http://localhost:8080/rtl/getAllRTL');
  ...
  const handleAddRoom = async () => {
    try {
      ...
      const newRTL = {
        rtlName: roomName, // Use the entered roomName
        // You can add other properties as needed
      };
  
      const response = await axios.post('http://localhost:8080/rtl/addRTL', newRTL);
      console.log('RTL added:', response.data);
      fetchData();
    } catch (error) {
      console.error('Error adding RTL:', error);
    }
  };
  ...
  const handleDeleteRoom = async (id) => {
    try {
      ...
      if (confirmDelete) {
        await axios.delete(`http://localhost:8080/rtl/deleteRTL/${id}`);
        console.log('RTL deleted:', id);
        // Update the state to trigger a re-render
        setFetchedData((prevData) => prevData.filter((room) => room.rtlId !== id));
      }
    } catch (error) {
      console.error('Error deleting RTL:', error);
    }
  };
              ...
              <div className="card text-black" style={{ backgroundColor: '#FFEBCD', ...cardStyles }}>
                ...
                {isEditing && (
                  <div className="card-footer" style={{ display: 'flex', justifyContent: 'space-between', padding: '10px' }}>
                    ...
                    <button
                      onClick={() => handleDeleteRoom(rtl.rtlId)}
